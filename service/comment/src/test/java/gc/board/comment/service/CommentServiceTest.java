...
class CommentServiceTest {
    ...
    private Comment createComment(Long articleId, Long commentId){
        ...
        given(comment.getParentCommentId()).willReturn(commentId);
        return comment;
    }
    ...
    @Test
    @DisplayName("하위 댓글이 삭제되고, 삭제표시 된 부모면, 재귀적으로 모두 삭제한다.")
    void deleteShouldDeleteAllRecursiveIfDeletedParent() {
        //given
        Long articleId = 1L;
        Long commentId = 2L;
        Long parentCommentId = 1L;
        ...
        // then
        verify(commentRepository).deleteById(commentId);
        verify(commentRepository).deleteById(parentCommentId);
    }
}